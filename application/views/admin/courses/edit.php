<main id="main" class="main">
	...
	<div class="card">
		<div class="card-body">
			...
			<form class="row g-3"  action="<?= base_url('courses/update/' . $course['id']) ?>" method="POST">
				<h5 class="card-title mx-2">Course Data</h5>

				<div class="col-md-6">
					<label class="form-label">Course Name<span class="text-red"></span><span
							class="text-danger">*</span></label>
					<input class="form-control" type="text" required name="name" value="<?= $course['name'] ?>" />
				</div>
				...
				<div class="col-md-6">
					...
					<select class="form-select" name="campus_id" required>
						...
						<?php foreach ($campus as $camp): ?>
							<option value="<?= $camp['id'] ?>" <?= ($camp['id'] == $course['campus_id']) ? 'selected' : '' ?>>
								<?= $camp['name'] ?></option>
						<?php endforeach; ?>
					</select>
				</div>
				...
				<div class="col-12 d-flex justify-content-end align-items-center">
					...
					<button class="btn btn-primary mt-2 ml-2" type="submit" name="submit">Update Course</button>
				</div>
